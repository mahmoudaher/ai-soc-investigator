...
class GraphWorkflowTests(unittest.IsolatedAsyncioTestCase):
    async def test_graph_runs_end_to_end_and_returns_casefile(self):
        ...
        case_file = (
            ...
            else CaseFile.model_validate(final_state)
        )

        self.assertIsInstance(case_file, CaseFile)

        # Finalizer should now set terminal state
        self.assertIn(case_file.status, {"completed", "failed"})
        ...
        # Finalizer does not add an AgentRun in your current design
        self.assertEqual(len(case_file.agent_runs), 3)

        # Order now matches graph: triage -> recon -> evidence
        self.assertEqual(case_file.timeline[0].agent, "triage_agent")
        self.assertEqual(case_file.timeline[1].agent, "recon_agent")
        self.assertEqual(case_file.timeline[2].agent, "evidence_agent")

        self.assertTrue(
            any(
                item.payload.get("validation") == "private_ip"
                for item in case_file.evidence
                if item.source == "recon_agent"
            )
        )
        self.assertTrue(
            any(
                item.payload.get("validation") == "lab_host"
                for item in case_file.evidence
                if item.source == "recon_agent"
            )
        )
