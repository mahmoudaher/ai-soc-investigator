...
const actionRoutes: Record<string, string> = {
  'view-cases': '/dashboard/cases',
  'workflow-board': '/dashboard/kanban',
  assistant: '/dashboard/chat'
};
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant='ghost' size='icon' className='relative h-8 w-8'>
          ...
          <span className='sr-only'>Workflow events</span>
        </Button>
      </PopoverTrigger>
      <PopoverContent align='end' className='w-[calc(100vw-2rem)] p-0 sm:w-[380px]' sideOffset={8}>
        <div className='flex items-center justify-between px-4 py-3'>
          <Link href='/dashboard/notifications' className='group flex items-center gap-1'>
            <h4 className='text-sm font-semibold group-hover:underline'>Workflow Events</h4>
